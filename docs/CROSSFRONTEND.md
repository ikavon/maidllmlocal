...
- [x] 措辞定稿（2026-09-18，对 MAICA 骨架语域核对后）：
      **L2 场景模板**（known_info 语域：第三人称/单句/句号/只给可验证事实）
      ——「(此刻莫妮卡身处 Minecraft 世界, 在这里她有一具身体.
      所在维度 {dimension}, 游戏内第{day}天, HH:00, 天气{晴/下雨/雷暴}. 她正处于{task}状态.
      她现在待在 {player_name} 身边.)」
      + 首访另起一行「(这是莫妮卡第一次进入这个存档.)」。
- [x] **时钟修订（2026-09-21 实测）**：现实深夜 + 游戏正午时她把游戏钟当成了现实钟
      （「中午了你怎么去睡觉」）——`第N天 14:00` 与 MFocus 每轮注入的现实时间**格式同构**，
      没有任何事实告诉她这是另一只跑得更快的钟。模板时间部分改为
      「这个世界的一天只有现实世界的20分钟. …这里的第{day}天, {相位}」：
      **只报相位不报钟点**（黎明/上午/正午/下午/黄昏/夜晚/深夜，与"HH:MM"形态不同构，分档见
      `MaicaScene.phaseOfDay`），**汇率单独成句**当世界事实给（她可拿去推理"我待了半天≈现实一小时"）。
      现实时间仍只归 MFocus 一个源，前端绝不报现实钟点（两边时区/精度一差，两个真相反杀比没有更糟）。
      ⚠️ 待游戏内实测。
